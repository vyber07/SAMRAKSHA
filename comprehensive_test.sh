...
echo "Starting comprehensive test..." > $REPORT
...
AUTH_RES=$(curl -s -X POST $BASE_URL/auth/login -H 'Content-Type: application/json' -d '{"badge_no":"ADMIN001","password":"admin123"}')
...
run_test "Login Invalid" "curl -s -w '\n%{http_code}' -X POST $BASE_URL/auth/login -H 'Content-Type: application/json' -d '{\"badge_no\":\"ADMIN001\",\"password\":\"wrong\"}'" "401"
run_test "Auth Me (Valid)" "curl -s -w '\n%{http_code}' -H 'Authorization: Bearer $TOKEN' $BASE_URL/auth/me" "200"
run_test "Auth Me (No Token)" "curl -s -w '\n%{http_code}' $BASE_URL/auth/me" "401"
