...
async def ask_assistant(
    ...
    """
    Ask the assistant a question with optional context
    
    Used by: Assistant service for case queries
    """
    messages = []
    ...
    llm = get_llm()
    return await llm.chat_completion(messages, system=system)
...
async def ask_chatbot(
    ...
    """
    Ask the chatbot a question with conversation history
    
    Used by: Chatbot service for conversational interaction
    """
    messages = conversation_history or []
    messages.append({"role": "user", "content": query})
    
    system = (
        "You are a helpful police assistant for the SAMRAKSHA system. "
        "Provide helpful, accurate information about police operations and cases. "
...
async def translate_text(
    ...
    """
    Translate text using LLaMA as fallback
    
    Used by: Translation service
    """
    prompt = (
        f"Translate the following text from {source_lang} to {target_lang}. "
        f"Reply ONLY with the translated text without any explanation:\n\n{text}"
    )
    
    llm = get_llm()
    return await llm.completion(prompt, temperature=0.0, max_tokens=200)
