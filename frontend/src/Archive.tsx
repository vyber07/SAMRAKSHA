...
export default function Archive() {
  ...
  useEffect(() => {
    ...
    axios.get(`${apiUrl}/docs/search`)
      .then(res => setData(res.data))
      .catch(() => {
        console.warn("Backend not running or auth required. Using mock data.");
        setData({total_documents: 145293, recently_accessed: 12, storage_used: "4.2 TB"});
      });
  }, []);
...
<li>
<Link className="bg-primary/20 text-white font-label-lg text-label-lg rounded-full p-4 flex items-center gap-4 transition-all duration-300 translate-x-1 shadow-sm border border-primary/30" to="/archive">
<span className="material-symbols-outlined text-primary" style={{ fontVariationSettings: "'FILL' 1" }}>archive</span> Archive</Link>
</li>
</ul>
...
<span className="material-symbols-outlined text-[20px]">add</span> New Mission
        ...
        <main className="flex-1 flex flex-col gap-6 md:h-[calc(100vh-160px)] min-h-[800px] md:min-h-[800px] xl:min-h-[600px]">
            <div className="glass-panel p-6 border-b border-white/10">
                <h2 className="font-headline-lg-mobile text-white">Document & Evidence Archive</h2>
                <p className="text-sm text-text-secondary mt-1 font-body-md">Historical records, legal docs, and closed FIRs</p>
            </div>
            ...
            <section className="flex-1 glass-panel p-6 overflow-y-auto custom-scrollbar">
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {data && Object.keys(data).length > 0 ? (
                        ...
                        <div className="col-span-full text-center text-text-secondary py-10">No data available from /docs/search</div>
                    )}
                </div>
            </section>
        </main>
