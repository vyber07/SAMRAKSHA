    ...
    axios.get(`${apiUrl}/cases/`)
      .then(res => {
        if (res.data && res.data.cases) {
            ...
            if (res.data.cases.length > 0) setSelectedCase(res.data.cases[0]);
        }
      })
      .catch(() => {
        console.warn("Backend not running or cases fetch failed.");
      });
  }, []);
...
<li>
<Link className="bg-primary/20 text-white font-label-lg text-label-lg rounded-full p-4 flex items-center gap-4 transition-all duration-300 translate-x-1 shadow-sm border border-primary/30" to="/cases">
<span className="material-symbols-outlined text-primary" style={{ fontVariationSettings: "'FILL' 1" }}>analytics</span> Cases</Link>
</li>
<li>
...
<span className="material-symbols-outlined text-[20px]">add</span> New Mission
        ...
        {/* Main Content Canvas */}
        <main className="flex-1 flex flex-col md:flex-row gap-6 md:h-[calc(100vh-160px)] md:min-h-[600px] min-h-[800px] md:min-h-0">
            
            {/* Left Column (FIR Directory) */}
            <aside className="w-full md:w-[35%] flex-shrink-0 flex flex-col min-h-[300px] md:min-h-0 md:h-full glass-panel overflow-hidden">
                <div className="p-6 border-b border-white/10">
                    <h2 className="font-headline-lg-mobile text-white">FIR Directory</h2>
                    <p className="text-sm text-text-secondary mt-1 font-body-md">Active investigations</p>
                </div>
                <div className="flex-1 overflow-y-auto px-4 py-2 custom-scrollbar">
                    {cases.map((c, i) => (
                        <div key={i} onClick={() => setSelectedCase(c)} className={`list-row py-3 px-3 hover:bg-white/10 transition-all cursor-pointer rounded-lg mb-1 ${selectedCase?.id === c.id ? "bg-white/10 border-white/20" : ""}`}>
                            <div className="flex justify-between items-start mb-1">
                                <span className="font-label-caps text-primary">{c.fir_number || c.id}</span>
                                <span className={`font-label-caps px-2 py-0.5 rounded-full text-[10px] ${c.status === "Closed" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-red-500/10 text-red-400 border border-red-500/20"}`}>{c.status?.toUpperCase() || "OPEN"}</span>
                            </div>
                            <div className="text-sm text-white font-medium truncate mb-1">{c.title || c.crime_type || "Investigation"}</div>
                            <div className="flex justify-between items-center text-xs text-text-secondary">
                                <span>IO: {c.investigating_officer || "Unassigned"}</span>
                                <span>{c.date_reported || c.time_str}</span>
                            </div>
                        </div>
                    ))}
                    {cases.length === 0 && <div className="text-center text-text-secondary py-10">No cases found.</div>}
                </div>
            </aside>

            {/* Right Area (Case Details) */}
            <section className="w-full md:w-[65%] min-h-[500px] md:min-h-0 md:h-full flex flex-col glass-panel overflow-hidden relative">
                {selectedCase ? (
                    <>
                    <div className="p-8 border-b border-white/10 flex justify-between items-start">
                        <div>
                            <div className="flex items-center gap-3 mb-2">
                                <span className="font-label-caps text-primary bg-primary/10 px-3 py-1 rounded-md border border-primary/20">{selectedCase.fir_number || selectedCase.id}</span>
                                <span className={`font-label-caps px-3 py-1 rounded-md ${selectedCase.priority === "High" ? "bg-red-500/10 text-red-400 border border-red-500/20" : "bg-amber-500/10 text-amber-400 border border-amber-500/20"}`}>{selectedCase.priority || "NORMAL"}</span>
                            </div>
                            <h1 className="font-headline-lg text-white mb-2">{selectedCase.title || selectedCase.crime_type || "Case Details"}</h1>
                            <p className="text-text-secondary text-sm flex items-center gap-2">
                                <span className="material-symbols-outlined text-sm">schedule</span> Registered: {selectedCase.date_reported || selectedCase.time_str}
                            </p>
                        </div>
                        <div className="flex gap-3">
                            <button className="bg-white/10 text-white px-4 py-2 rounded-lg font-medium text-sm hover:bg-white/20 transition-colors flex items-center gap-2">
                                <span className="material-symbols-outlined text-[18px]">person_add</span> Assign
                            </button>
                            <button className="bg-primary text-white px-4 py-2 rounded-lg font-medium text-sm hover:brightness-110 transition-colors shadow-none flex items-center gap-2 border border-primary">
                    ...
                    <div className="flex-1 overflow-y-auto p-8 flex flex-col gap-8 custom-scrollbar">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="flex flex-col gap-2">
                                <label className="font-label-caps text-text-secondary">Investigating Officer</label>
                                <div className="bg-black/40 border border-white/10 rounded-lg p-3 flex items-center gap-3">
                                    <div className="flex flex-col">
                                        <span className="text-sm font-medium text-white">{selectedCase.investigating_officer || "Unassigned"}</span>
                                    </div>
                                </div>
                            </div>
                            <div className="flex flex-col gap-2">
                                <label className="font-label-caps text-text-secondary">Date of Incident</label>
                                <div className="bg-black/40 border border-white/10 rounded-lg p-3 flex items-center gap-3 h-[58px]">
                                    <span className="material-symbols-outlined text-primary">event</span>
                                    <span className="text-sm text-white">{selectedCase.date_incident || "N/A"}</span>
                                </div>
                            </div>
                            <div className="flex flex-col gap-2">
                                <label className="font-label-caps text-text-secondary">Location</label>
                                <div className="bg-black/40 border border-white/10 rounded-lg p-3 flex items-center gap-3 h-[58px]">
                                    <span className="material-symbols-outlined text-primary">location_on</span>
                                    <span className="text-sm text-white truncate">{selectedCase.location || "N/A"}</span>
                                </div>
                            </div>
                        </div>

                        <div className="flex flex-col gap-4">
                            <h3 className="font-headline-lg-mobile text-white border-b border-white/10 pb-2">Initial Report</h3>
                            <div className="bg-black/40 border border-white/10 rounded-xl p-5">
                                <p className="text-sm text-slate-300 leading-relaxed">
                                    {selectedCase.description || "No description available for this case."}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 flex items-center justify-center text-text-secondary">Select a case to view details</div>
                )}
            </section>
        </main>
      </div>
