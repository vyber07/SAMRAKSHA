...
describe('React Component State Transitions & Zustand Store Tests', () => {
  ...
  test('IncidentFrequencyChart Data Normalization & State Machine', () => {
    // 1. Chart Controls state machine simulation
    let chartType = 'area';
    let granularity = 'weekly';
    const setChartType = (t) => { chartType = t; };
    const setGranularity = (g) => { granularity = g; };

    assert.equal(chartType, 'area');
    assert.equal(granularity, 'weekly');

    setChartType('bar');
    setGranularity('hourly');
    assert.equal(chartType, 'bar');
    assert.equal(granularity, 'hourly');

    setChartType('line');
    setGranularity('monthly');
    assert.equal(chartType, 'line');
    assert.equal(granularity, 'monthly');

    // 2. Data shape normalization testing
    const sampleTrends = {
      hourly: [{ hour: 14, count: 18 }, { hour: 20, count: 25 }],
      weekly: [{ day: 'Mon', count: 42 }, { day: 'Thu', count: 74 }],
      monthly: [{ month: 'Jan', count: 180 }, { month: 'Dec', count: 340 }],
    };

    // Hourly slot mapping (24 slots)
    const hourlySlots = Array.from({ length: 24 }, (_, i) => ({
      label: `${String(i).padStart(2, '0')}:00`,
      hour: i,
      count: 0,
    }));
    sampleTrends.hourly.forEach((item) => {
      if (item.hour >= 0 && item.hour < 24) {
        hourlySlots[item.hour].count = item.count;
      }
    });
    assert.equal(hourlySlots.length, 24);
    assert.equal(hourlySlots[14].count, 18);
    assert.equal(hourlySlots[20].count, 25);
    assert.equal(hourlySlots[0].count, 0);

    // Weekly day mapping (7 slots)
    const daysMonToSun = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
    const weeklyMap = { Mon: 0, Tue: 0, Wed: 0, Thu: 0, Fri: 0, Sat: 0, Sun: 0 };
    sampleTrends.weekly.forEach((item) => {
      if (weeklyMap.hasOwnProperty(item.day)) {
        weeklyMap[item.day] = item.count;
      }
    });
    const weeklyPoints = daysMonToSun.map((day) => ({ label: day, count: weeklyMap[day] }));
    assert.equal(weeklyPoints.length, 7);
    assert.equal(weeklyPoints[0].count, 42);
    assert.equal(weeklyPoints[3].count, 74);
    assert.equal(weeklyPoints[1].count, 0);

    // Monthly month mapping (12 slots)
    const monthsJanToDec = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
    const monthlyMap = Object.fromEntries(monthsJanToDec.map((m) => [m, 0]));
    sampleTrends.monthly.forEach((item) => {
      if (monthlyMap.hasOwnProperty(item.month)) {
        monthlyMap[item.month] = item.count;
      }
    });
    const monthlyPoints = monthsJanToDec.map((m) => ({ label: m, count: monthlyMap[m] }));
    assert.equal(monthlyPoints.length, 12);
    assert.equal(monthlyPoints[0].count, 180);
    assert.equal(monthlyPoints[11].count, 340);
    assert.equal(monthlyPoints[1].count, 0);
  });
