 ...
 * Author: Challenger 1 Gen 2 (challenger_m4_1_gen2)
 */
...
export async function runChallengerM41Gen2Tests() {
  console.log('===================================================================');
  console.log('   CHALLENGER 1 GEN 2: EMPIRICAL TEST SUITE FOR MILESTONE 4 VIEWS  ');
  console.log('===================================================================');
  ...
  await runTest('Filter FIRs by status = "pending" returns exact matching FIR records', async () => {
    ...
    const filtered = allCases.filter((c) => c.status === 'pending');
  ...
  await runTest('Filter FIRs by status = "closed" returns exact matching FIR records', async () => {
    const allCases = await casesApi.getCases();
    const filtered = allCases.filter((c) => c.status === 'closed');

    assert(filtered.length > 0, 'Should return closed FIRs');
    assert(
      filtered.every((c) => c.status === 'closed'),
      'All returned FIRs must have status equal to closed'
    );
  });
...
runChallengerM41Gen2Tests().catch((err) => {
